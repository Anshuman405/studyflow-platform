...
import { CheckSquare, Calendar, FileText, TrendingUp, Clock, Target, Zap, ArrowRight, Plus } from "lucide-react";
...
function DashboardContent() {
  ...
  const { data: tasks, isLoading: tasksLoading } = useOptimizedQuery({
    queryKey: ["tasks"],
    queryFn: () => backend.tasks.list({ limit: 20 }),
    staleTime: 2 * 60 * 1000, // 2 minutes
  });

  const { data: events, isLoading: eventsLoading } = useOptimizedQuery({
    queryKey: ["events"],
    queryFn: () => backend.events.list({ limit: 20 }),
    staleTime: 2 * 60 * 1000,
  });
  ...
  const { data: notes, isLoading: notesLoading } = useOptimizedQuery({
    queryKey: ["notes"],
    queryFn: () => backend.notes.list({ limit: 10 }),
    staleTime: 5 * 60 * 1000,
  });
